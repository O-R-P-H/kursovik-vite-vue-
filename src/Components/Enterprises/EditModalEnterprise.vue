...
export default {
  ...
  emits: ['close', 'enterprise-updated'],
  setup(props, { emit }) {
    const formData = ref({ ...props.enterpriseToEdit });
    const isLoading = ref(false);
    const errors = ref([]);

    watch(() => props.enterpriseToEdit, (newVal) => {
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        const updatedEnterprise = await ManufacturerApi.update(id, updateData);
        emit('enterprise-updated', updatedEnterprise);
        closeModal();
      } catch (error) {
        console.error('Ошибка при обновлении:', error);
        alert(error.response?.data?.message || 'Не удалось обновить предприятие');
      } finally {
